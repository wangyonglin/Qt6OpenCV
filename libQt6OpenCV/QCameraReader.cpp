...
Qt6OpenCV::QCameraReader::QCameraReader(QObject *parent)
    : QCameraThreader(parent)
{

}

Qt6OpenCV::QCameraReader::~QCameraReader()
{

}

void Qt6OpenCV::QCameraReader::setCameraWidth(int width)
{
    camera_width=width;
}

void Qt6OpenCV::QCameraReader::setCameraHeight(int height)
{
     camera_height=height;

}

void Qt6OpenCV::QCameraReader::setCameraFramerate(int framerate)
{
     camera_framerate=framerate;
}

bool Qt6OpenCV::QCameraReader::open(QString name)
{
...
//    qDebug() << "pipeline:" <<pipeline;
    ...
    yuv420_size =  camera_width*camera_height*3/2;
    yuv420_data=make_unique<unsigned char[]>(yuv420_size);
    emit sigFirst(yuv420_data.get(),camera_width,camera_height);
    start();
    return true;
}

void Qt6OpenCV::QCameraReader::close()
{
    stop();
    if(capture)
    {
...
void Qt6OpenCV::QCameraReader::loop()
{
    //            Mat hsvframe, mask;
    ...
    if(capture->isOpened())
    {
        ...
        {
            cv::Mat yuvImg;
            cv::cvtColor(dstMat, yuvImg, cv::COLOR_BGR2YUV_I420);
            memcpy(yuv420_data.get(), yuvImg.data, yuv420_size*sizeof(unsigned char));
            emit refresh();
        }else{
           stop();
        }
...
QPixmap Qt6OpenCV::QCameraReader::scaled2QImage(const QImage & image)
{
    QImage scaledImage =image.scaled(32, 32, Qt::KeepAspectRatio, Qt::SmoothTransformation);
    return QPixmap::fromImage(scaledImage);
}

Mat Qt6OpenCV::QCameraReader::InRangeHSV(const Mat &frame,Mat &mask,Scalar & lowerb,Scalar & upperb)
{
    Mat hsv;
    //将每一帧的图像转换到hsv空间
...
Mat Qt6OpenCV::QCameraReader::Matting(Mat frame, Mat mask)
{
...
Mat Qt6OpenCV::QCameraReader::Replace(Mat frame, Mat mask, Mat bg)
{
...
void Qt6OpenCV::QCameraReader::yuyvToRgb(unsigned char *yuv_buffer,unsigned char *rgb_buffer,int iWidth,int iHeight)
{
    int x;
    int z=0;
 ...
 */
void Qt6OpenCV::QCameraReader::nv21ToRgb24(unsigned char *yuyv, unsigned char *rgb, int width, int height)
{
...
QImage Qt6OpenCV::QCameraReader::cvMatToQImage(const cv::Mat &inMat) {
    switch (inMat.type()) {
    // 8-bit, 4 channel
    case CV_8UC4: {
