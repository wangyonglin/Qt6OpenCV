#include "QCameraPlayer.h"

#include <QDebug>


Qt6OpenCV::QCameraPlayer::QCameraPlayer(QWidget *parent)
    :QOpenGLWidget(parent)
{
    reader= new QCameraReader(this);

    connect(reader,&QCameraReader::sigFirst,[=](uchar* p,int w,int h){
        ptr = p;
        width = w;
        height = h;
    });
    connect(reader,&QCameraReader::refresh,[=](){
        update();
    });
}

Qt6OpenCV::QCameraPlayer::~QCameraPlayer()
{

}
void Qt6OpenCV::QCameraPlayer::play(const QString &device)
{
    reader->open(device);
}

void Qt6OpenCV::QCameraPlayer::stop()
{

    reader->close();
}

void Qt6OpenCV::QCameraPlayer::initializeGL()
{
...
void Qt6OpenCV::QCameraPlayer::resizeGL(int w, int h)
{
    if(h<=0) h=1;
...
void Qt6OpenCV::QCameraPlayer::paintGL()
{
    if(!ptr) return;
